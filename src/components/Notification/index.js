...
const NotificationItem = ({
                              level = "low", title, groupLength, subtitle, content, link, id, children, className,
                          }) => {
    const {setGlobal} = useGlobalContext("notification");
    const {apis, ajax} = usePreset();
    const {isLoading, callback} = useLoading(async () => {
        const {data: resData} = apis?.notification?.read ? await ajax(Object.assign({}, apis.notification.read, {
            data: {ids: [id]},
        })) : {data: {code: 0}};
    ...
    >
        {groupLength && groupLength > 1 && range(1, Math.min(groupLength, 3))
            .reverse()
            .map((number) => (<div
                className={classnames(style["item-box"], style[`index-${number}`])}
                key={number}
            />))}
        <div className={classnames(style["item-box"], style["item-box-main"])}>
            ...
            <div className={style["item-content"]}>
                ...
                {Array.isArray(content) ? (<Content list={content} labelAlign="auto" size="small"/>) : (children)}
            </div>
            {link && (<div className={style["link-outer"]}>
                <LoadingButton
...
const Notification = ({itemClassName}) => {
    ...
    useEffect(() => {
        headOptionRef.current && setHeadOptionHeight(headOptionRef.current?.clientHeight);
    }, []);
    ...
    >
        <div ref={headOptionRef}>
            <Row justify="end" wrap={false} className={style["header"]} gutter={8}>
                {groupList.some((item) => item.length > 1) && (<Col>
                    <Button
                        size="small"
                        onClick={() => {
                            setGlobal(Object.assign({}, notification, {expand: !expand}));
                        }}
                    >
                        {expand ? "全部折叠" : "全部展开"}
                    </Button>
        ...
        <SimpleBar className={style["list"]}>
            <QueueAnim leaveReverse duration={200} interval={20}>
                {groupList
                    ...
                    .map((item, groupKey) => {
                        if (expand) {
                            return item.map((notification, index) => notification && (
                                <div key={`group-${groupKey}-${index}`}>
                                    <NotificationItem
                                        {...Object.assign({}, notification)}
                                        className={classnames({
                                            [style["is-last-item"]]: +groupKey + 1 === Math.min(groupList.filter((item) => item && item.length)?.length, 3) && index === item?.length - 1,
                                        })}
                                    />
                                </div>));
                        }
                        const firstItem = first(item);
                        return (<div key={`group-${groupKey}-0`}>
                            <NotificationItem
                                {...Object.assign({}, firstItem)}
                                groupLength={item.length}
                                className={classnames({
                                    [style["is-last-item"]]: groupList?.length && +groupKey + 1 === Math.min(groupList.filter((item) => item && item.length)?.length, 3),
                                }, itemClassName)}
                            />
                        </div>);
                    })}
