...
const BaseExample = () => {
    ...
    return (<Space direction="vertical">
        <Radio.Group
            value={size}
            options={[{label: "small", value: "small"}, {label: "default", value: "default"}, {
        ...
        <Space wrap>
            ...
            <Button
                onClick={() => {
                    modal({
                        title: "hooks调用弹框", size, children: <div>弹窗弹窗弹窗弹窗弹窗弹窗弹窗</div>,
                    });
                }}
            >
                hooks调用弹框
            ...
            <Button
                onClick={() => {
                    modal({
                        title: "超高弹窗", size, children: (<div style={{height: "2000px"}}>
            ...
            <Button
                onClick={() => {
                    modal({
                        title: "无footer弹框", children: <div>弹窗弹窗弹窗弹窗弹窗弹窗弹窗</div>, footer: null,
                    });
                }}
            >
                无footer弹框
            ...
            <Button onClick={() => {
                ...
                modal({
                    title: "有rightOptions的弹窗", size, withDecorator: (render) => {
                        return <StateContainer>{render}</StateContainer>
                    }, footerButtons: ({disabled}) => [{
                        type: 'primary', disabled, children: '确定'
                    }], children: ({disabled, setDisabled}) => <div>
                        弹窗弹窗弹窗弹窗弹窗弹窗弹窗[{String(disabled)}]
                        <Button onClick={() => {
                            setDisabled((disabled) => !disabled);
                        }}>切换确定按钮disabled</Button>
                    </div>,
                });
            }}>children控制footerButtons状态</Button>
        </Space>
    </Space>);
};
