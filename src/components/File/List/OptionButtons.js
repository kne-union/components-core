...
const OptionButtons = createWithIntl({
    ...
    return (<Space size={0}>
        {hasPreview && getPermission("preview", item) && (<Button
            type="text"
            icon={<Icon type="icon-yulan"/>}
            onClick={() => {
                apis.onPreview ? apis.onPreview(item) : fileModal({id, originName: filename, apis});
            }}
        />)}
        {getPermission("edit", item) && (<Button
            type="text"
        ...
        />)}
        {getPermission("download", item) && (<DownloadButton type="text" id={id} filename={filename}/>)}
        {getPermission("delete", item) && (<ConfirmButton
            type="text"
