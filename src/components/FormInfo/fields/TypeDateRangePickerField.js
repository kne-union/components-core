...
const TypeDateRangePickerField = ({...props}) => {
            ...
            <Select
                ...
                onChange={(typeValue) => {
                    onChange((value) => {
                        ...
                        const startTime = dayjs(value.value[0]),
                            endTime = dayjs(value.value[1]);
                        return {
                            value: [
                                startTime.isValid() ? startTime.startOf(typeValue || "date") : null,
                                endTime.isValid() ? endTime.endOf(typeValue || "date") : null,
                            ],
                            type: typeValue || "date",
                        };
                    });
                    ...
                    onChange((value) => {
                        return Object.assign({type: "date"}, value, {
                            value:
                                dateValue && [
                                    dateValue[0] &&
                                        new Date(dateValue[0].startOf(value?.type || "date").valueOf()),
                                    dateValue[1] &&
...
export default withLocale(TypeDateRangePickerField);
